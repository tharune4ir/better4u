...
export default function LandingHub() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F7F6F2] relative overflow-hidden select-none font-sans text-slate-800">
      ...
      {/* ==================== ACT 5: THE ENGINE (BUILD IN PUBLIC) ==================== */}
      <section className="relative min-h-screen w-full flex flex-col justify-center items-center px-6 py-20 bg-white/40 border-y border-black/[0.01]">
        <div className="max-w-4xl w-full space-y-12 z-10 relative">
          ...
          <div className="text-center space-y-3">
            ...
            <h2 className="text-3xl font-extralight text-slate-900 tracking-tight">
              A transparent <span className="font-semibold text-slate-950">engineering ledger</span>
            </h2>
            <p className="text-xs text-slate-500 font-light max-w-md mx-auto leading-relaxed">
              We craft every line of code and curate each gut-friendly food SKU out in the open. Follow our design cycles, refactors, and project milestones.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
            
            {/* Interactive Timeline Graph Graphic */}
            <div className="bg-[#F7F6F2]/60 border border-black/[0.03] rounded-3xl p-6 flex flex-col justify-between relative overflow-hidden shadow-inner">
              <div className="absolute top-4 right-4 flex items-center gap-1.5">
                <div className="w-2 h-2 rounded-full bg-[#2A7F7F] animate-pulse" />
                <span className="text-[9px] tracking-wider text-slate-400 font-bold uppercase">Repo Active</span>
              </div>
              
              <div>
                <span className="text-[9px] text-[#2A7F7F] font-bold tracking-widest uppercase">Live Nodes</span>
                <h3 className="text-base font-bold text-slate-900 mt-1">Commit Pulse Map</h3>
              </div>

              {/* Node SVG visualization */}
              <div className="w-full h-48 my-4 flex items-center justify-center">
                <svg viewBox="0 0 300 120" className="w-full h-full overflow-visible">
                  {/* Grid Lines */}
                  <line x1="10" y1="60" x2="290" y2="60" stroke="#e2e8f0" strokeDasharray="3,3" />
                  ...
                  <motion.path 
                    ...
                    d="M 20 60 Q 60 100, 100 60 T 180 60 T 260 60" 
                    fill="none" 
                    stroke="#2A7F7F" 
                    strokeWidth="1.5" 
                  />
                  ...
                  <g className="cursor-pointer group">
                    <circle cx="20" cy="60" r="4" fill="#2A7F7F" />
                    <motion.circle cx="20" cy="60" r="10" fill="none" stroke="#2A7F7F" strokeWidth="1" animate={{ scale: [1, 1.8, 1], opacity: [0.6, 0, 0.6] }} transition={{ repeat: Infinity, duration: 3 }} />
                  </g>

                  {/* Node 2: Gut Pillar */}
                  <g className="cursor-pointer group">
                    <circle cx="100" cy="60" r="4" fill="#2A7F7F" />
                    <motion.circle cx="100" cy="60" r="10" fill="none" stroke="#2A7F7F" strokeWidth="1" animate={{ scale: [1, 1.8, 1], opacity: [0.6, 0, 0.6] }} transition={{ repeat: Infinity, duration: 3, delay: 0.5 }} />
                  </g>

                  {/* Node 3: Refactor */}
                  <g className="cursor-pointer group">
                    <circle cx="180" cy="60" r="4" fill="#2A7F7F" />
                    <motion.circle cx="180" cy="60" r="10" fill="none" stroke="#2A7F7F" strokeWidth="1" animate={{ scale: [1, 1.8, 1], opacity: [0.6, 0, 0.6] }} transition={{ repeat: Infinity, duration: 3, delay: 1 }} />
                  </g>

                  {/* Node 4: Product Lab */}
                  <g className="cursor-pointer group">
                    <circle cx="260" cy="60" r="5" fill="#1e5c5c" />
                    <motion.circle cx="260" cy="60" r="12" fill="none" stroke="#1e5c5c" strokeWidth="1.5" animate={{ scale: [1, 2, 1], opacity: [0.8, 0, 0.8] }} transition={{ repeat: Infinity, duration: 2.5 }} />
                  </g>
                  
                  {/* Labels on SVG */}
                  <text x="20" y="45" fill="#94a3b8" fontSize="7" fontWeight="bold" textAnchor="middle" letterSpacing="0.05em">INIT</text>
                  <text x="100" y="80" fill="#94a3b8" fontSize="7" fontWeight="bold" textAnchor="middle" letterSpacing="0.05em">DATABASE</text>
                  <text x="180" y="45" fill="#94a3b8" fontSize="7" fontWeight="bold" textAnchor="middle" letterSpacing="0.05em">REFACTOR</text>
                  <text x="260" y="80" fill="#2A7F7F" fontSize="7" fontWeight="bold" textAnchor="middle" letterSpacing="0.05em">LIVE LAB</text>
                </svg>
              </div>

              <div className="text-[10px] text-slate-400 font-light leading-relaxed">
                Our code nodes represent core design iterations, database structures, and styling optimizations.
              </div>
            </div>

            {/* Live Terminal logs block */}
            <div className="bg-slate-950 text-slate-300 rounded-3xl p-6 font-mono text-xs flex flex-col justify-between shadow-2xl relative">
              {/* Terminal header */}
              <div className="flex items-center justify-between pb-4 border-b border-white/5 mb-4">
                ...
                <span className="text-[10px] text-slate-500 uppercase tracking-widest">trelis-life@git-ledger</span>
              </div>

              {/* Logs */}
              <div className="space-y-3.5 flex-1 select-text">
                <div className="flex items-start gap-2">
                  <span className="text-[#2A7F7F]">➜</span>
                  <div>
                    <span className="text-slate-500">[06-29]</span> <span className="text-slate-200">refactor:</span> open-sourced sku store access; shifted <code className="text-[#2A7F7F]">/vault</code> to <code className="text-emerald-400">/product-lab</code>.
                  </div>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-[#2A7F7F]">➜</span>
                  <div>
                    <span className="text-slate-500">[06-29]</span> <span className="text-slate-200">design:</span> stripped movement modules, pivoted 100% to gut health food line.
                  </div>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-[#2A7F7F]">➜</span>
                  <div>
                    <span className="text-slate-500">[06-23]</span> <span className="text-slate-200">feat:</span> integrated daily seed & ferment intake log store.
                  </div>
                </div>
                <div className="flex items-start gap-2">
                  <span className="text-[#2A7F7F]">➜</span>
                  <div>
                    <span className="text-slate-500">[06-16]</span> <span className="text-slate-200">init:</span> launched hub v1.0 under off-white glassmorphic system.
                  </div>
                </div>
              </div>

              <div className="pt-4 border-t border-white/5 mt-4 flex items-center justify-between text-[9px] text-slate-500">
