...
// SET THIS TO true TO BYPASS LOCK SCREEN FOR TESTING. SET TO false TO RESTORE PASSCODE ENFORCEMENT.
...
export default function LockScreen({ children }: LockScreenProps) {
  ...
  const handlePasswordChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val === "rudra22") {
      ...
      // Trigger error shake if incorrect after 7 characters
      setIsError(true);
      setTimeout(() => {
        setIsError(false);
        setPassword("");
    ...
    <>
      <AnimatePresence mode="wait">
        {!isUnlocked && (
          ...
          >
            {/* Ambient Background Glows tailored to brand-teal */}
            <div className="absolute top-1/4 left-1/4 w-[300px] h-[300px] sm:w-[400px] sm:h-[400px] bg-gradient-to-tr from-brand-teal/15 to-emerald-200/10 rounded-full blur-[80px] sm:blur-[100px] pointer-events-none" />
            <div className="absolute bottom-1/4 right-1/4 w-[350px] h-[350px] sm:w-[500px] sm:h-[500px] bg-gradient-to-tr from-slate-200/20 to-brand-teal/5 rounded-full blur-[100px] sm:blur-[120px] pointer-events-none" />
            ...
            >
              ...
              >
                ...
                <div className="absolute bottom-4 w-36 h-32 sm:w-40 sm:h-36 rounded-[24px] sm:rounded-[28px] glassmorphic p-[1px] shadow-2xl flex items-center justify-center overflow-hidden">
                  ...
                  <div className="absolute bottom-4 right-4 w-20 h-20 bg-gradient-to-br from-brand-teal/15 to-emerald-400/5 rounded-full blur-xl pointer-events-none" />
                ...
                {/* Ring Ambient Overlay */}
                <div className="absolute -inset-2 rounded-full border border-black/[0.01] pointer-events-none" />
              </motion.div>

              {/* Title & Micro-copy */}
              <h1 className="text-xl sm:text-2xl font-light tracking-[0.25em] text-brand-teal uppercase mb-2">
                Trelis Life
              </h1>
              ...
              <div className="relative w-40 sm:w-48 h-12 flex items-center justify-center">
                {/* Real Input (Hidden visually but focusable, at least 44px high) */}
                <input
                  ref={inputRef}
                  type="password"
                  value={password}
                ...
                <div className="absolute inset-x-0 bottom-0 top-0 flex items-center justify-center gap-4 border-b border-brand-teal/20 pb-2 z-10 pointer-events-none">
                  {password.length === 0 ? (
                  ...
                  {/* Custom Blinking Caret in Brand Teal */}
                  <div className="w-[2px] h-4 bg-brand-teal custom-cursor" />
                </div>
              </div>
              ...
              <motion.div 
                ...
                animate={{ opacity: 0.4 }}
                transition={{ delay: 2 }}
                className="mt-12 sm:mt-16 text-[10px] tracking-widest text-slate-400 font-light"
              >
                Hint: rudra22
              </motion.div>
            </motion.div>
